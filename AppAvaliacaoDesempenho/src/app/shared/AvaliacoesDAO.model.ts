...
export class AvaliacoesDAO{

    private AvaliacoesAux:Avaliacao[];

    constructor(private BD: BDService, private autenticacao: Autenticacao){
        
    }

public GetDados(fn, filtrarLogado = false){
this.AvaliacoesAux = [];
let aux = [];
...
this.BD.recuperarArrayAvaliacao().subscribe((resposta: Avaliacao[]) => {
  ...
  this.AvaliacoesAux = aux;
  this.autenticacao.retornaUsuarioLogado().then((usuario: any) => {
    var aval = this.AvaliacoesAux;
    if(!usuario.email.toUpperCase().includes("GERENTE") || filtrarLogado){
      aval = aval.filter(x => x.Vendedor.ID == usuario.email);
    }
    fn(aval);
  });
});
